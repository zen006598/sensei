...
- **`src/agent/register_classifier.py`** — `RegisterClassifier.classify(card) -> str`. LLM-backed only. Checks the Anki note for an existing `sensei:<register>` tag first; if missing, calls `agent.classify_register` and writes the tag back. State machine receives this and a `FrequencyClassifier` separately — they don't share a combined return type.
- **`src/word_service/frequency.py`** — `FrequencyClassifier.classify(word) -> str`. Pure-function-ish: `wordfreq.zipf_frequency` → `_bucket` returns common/rare/obsolete. No LLM, no I/O, no `async`. Lives outside `src/agent/` because it's not LLM-related; the agent layer should not be coupled to local linguistic services. `zipf_fn` is constructor-injectable for tests.
