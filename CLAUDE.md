...
- **`src/agent/word_classifier.py`** — `WordClassifier.classify(card)` returns `WordClassification(frequency, register)`. Frequency is looked up via the local `wordfreq` package (Zipf score → bucketed into common/rare/obsolete by `_bucket`) — no API call, no cache. Register still comes from Gemini (`agent.classify_register`) and is cached as a `sensei:<register>` tag on the Anki note so subsequent quizzes skip the LLM call. `zipf_fn` is constructor-injectable for tests.
